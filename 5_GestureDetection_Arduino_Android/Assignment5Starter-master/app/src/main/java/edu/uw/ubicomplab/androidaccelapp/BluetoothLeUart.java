...
public class BluetoothLeUart extends BluetoothGattCallback implements BluetoothAdapter.LeScanCallback {
    ...
    private Semaphore writeInProgress;
...
//        this.writeInProgress = false;
        this.writeInProgress = new Semaphore(0);
...
//        } // Wait for the flag to clear in onCharacteristicWrite
        ...
        if (newState == BluetoothGatt.STATE_CONNECTED) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                if (!gatt.discoverServices()) {
                    ...
                    Log.e("BLE-UART", "Error starting service discovery");
                    connectFailure();
                }
            }
            else {
                // Error connecting to device.
                Log.e("BLE-UART", "Error connecting to device");
                connectFailure();
            }
        }
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        if (status == BluetoothGatt.GATT_FAILURE) {
            Log.e("BLE-UART", "service discovery failed");
            connectFailure();
            return;
        }
        ...
        if (!gatt.setCharacteristicNotification(rx, true)) {
            ...
            Log.e("BLE-UART", "characteristic notification setup failed");
            connectFailure();
            return;
        }
        ...
        if (desc == null) {
            ...
            Log.e("BLE-UART", "RX characteristic has no client descriptor");
            connectFailure();
            return;
        }
        ...
        if (!gatt.writeDescriptor(desc)) {
            ...
            Log.e("BLE-UART", "client descriptor could not be written");
            connectFailure();
            return;
        }
    ...
    public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        if (status == BluetoothGatt.GATT_SUCCESS) {
            Log.d("BLE-UART","Characteristic write successful");
        }
