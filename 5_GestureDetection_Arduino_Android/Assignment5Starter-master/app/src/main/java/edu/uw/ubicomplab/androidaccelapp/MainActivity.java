...
public class MainActivity extends AppCompatActivity implements BluetoothLeUart.Callback {
    ...
    public boolean continueStrFlag = false;
    ...
    // Machine learning
    private Model model;
    private static final int GESTURE_DURATION_SECS = 2; //Changed The Time Duration to 2s
    ...
    private static final String DEVICE_NAME = "Anusha";
    ...
    /*
     NOTE : This Function is used to safely disconnect the connection of phone with Arduino using the button Disconnect
     */
     ...
     */
    /*
    NOTE : The function is used to establish a start-stop command with Arduino
    When a gesture buttons or Test Button is clicked,
    A start command -"A" is sent to Arduino
    After a time of GESTURE_DURATION_SECS in this case 2sec,
    A stop signal -"Z" is sent.

    Arduino gathers the motion values in between A-Z (commands) and sends the calculated features
     */
...
//        startTimer.schedule(startTask, 0);
    ...
    /*
    NOTE : The main receiving function is written in the below method
    When Arduino sends the calculated features, This method is to receive the data.

    Steps/Concept:
    1. Problem : Bluetooth can only send 20Bytes at once , if there is more data
    It breaks it down and sends in multiple packets.
    Solution : A header and end character were sent . Also delimiters were sent to distinguish
    different features.

    2. Even if a header and end charcters were sent, the whole message was still broken into pieces
    Solution : A Flag was used - continueStrFlag ; which estentially checks if the whole message is received
    - Check point for this is end character is received.

    3. Now the code works for both cases - If message is recieved all at once or in pieces.

    4. Partial Messages are captured in RxStr and eventually whole message is stored in RxBuffer

    5. Notation : 'o'+feature1+','+feature2+','+feature3+','+feature4+','+feature5+','+feature6+'x'
    Header Character : 'o'
    End Character  : 'x'
    Delimiters : ','
    Feature 1 : Max of ax
    Feature 2 : Max of ay
    Feature 3 : Max of az
    Feature 4 : Mean of ax
    Feature 5 : Mean of ay
    Feature 6 : Mean of az

    6. Processing of the received message is done in 2 steps
    a. Remove header and end character
    b. Substring removing delimiters

     */
    @Override
    public void onReceive(BluetoothLeUart uart, BluetoothGattCharacteristic rx) {

        String RxStr = rx.getStringValue(0);             //Accepting the values from UART

        //Reformation of valid message - check for all valid parts and reconstruct the message sent

        if (continueStrFlag)                                  // still accepting the parts of message till last charcter x
        {
            RxBuffer = RxBuffer + RxStr;
        } else { // looking for new msg
            if (RxStr.charAt(0) == 'o')                         //start point of message acceptance
            {
                RxBuffer = RxStr;
                continueStrFlag = true;
            }
        }
        if(RxStr.charAt(RxStr.length()-1) == 'x')               //Stop receiving if last charcter is found
        {
            continueStrFlag = false;
        }

        // Processing the message and extracting features from the message.

        if (!continueStrFlag)
        {
            writeLine("RxBuffer: " + RxBuffer);
            String processString = RxBuffer.substring(1, (RxBuffer.length() - 1));
            String[] featureString = processString.split(",");

            // check if split is correct and 6 features are captured.
            // writeLine("length of feature string after split:" + featureString.length);

            double[] features = new double[6];                  // TODO: convert your received string into a double[]
            for (int i = 0; i < featureString.length; i++)
            {
                features[i] = Double.valueOf(featureString[i]);
            }

            writeLine("features double" + features[0] + "," +  features[1] + "," + features[2]+"," + features[3]+"," + features[4]+"," + features[5]);
